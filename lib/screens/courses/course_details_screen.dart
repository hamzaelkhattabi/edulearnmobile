...
class _CourseDetailsScreenState extends State<CourseDetailsScreen> {
  ...
  double _progress = 0.0;   // TODO: A récupérer via API si l'utilisateur est inscrit4
  ...
  void initState() {
    ...
    _fetchFullCourseDetailsAndEnrollmentStatus();
  }

  Future<void> _fetchFullCourseDetailsAndEnrollmentStatus() async {
    if (mounted) setState(() { _isLoadingDetails = true; _isCheckingEnrollment = true; });
    try {
      // 1. Charger les détails du cours (si nécessaire, sinon _course = widget.courseInput suffit)
      ...
      // 2. VÉRIFIER LE STATUT D'INSCRIPTION (UNIQUEMENT SI UN UTILISATEUR EST CONNECTÉ)
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      if (authProvider.isAuthenticated) {
        // Supposons une méthode dans EnrollmentService pour cela:
        // getMyEnrollmentForCourse renvoie EnrollmentModel ou null
        final EnrollmentModel? enrollment = await _enrollmentService.getMyEnrollmentForCourse(_course.id);
        if (mounted && enrollment != null) {
          setState(() {
            _currentEnrollmentId = enrollment.id;
            // _isEnrolled = true; // Vous avez déjà cette logique dans _enrollOrContinueCourse
            // _progress = enrollment.progressionPourcentage;
          });
        } else if (mounted) {
          setState(() {
            _currentEnrollmentId = null; // Pas d'inscription trouvée
          });
        }
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Erreur chargement détails/inscription: ${e.toString()}"), backgroundColor: eduLearnError),
        );
      }
    } finally {
      if (mounted) {
        setState(() { _isLoadingDetails = false; _isCheckingEnrollment = false; });
      }
    }
  }

   Widget _buildChaptersList(List<LessonModel> lessons) {
    if (_isCheckingEnrollment && Provider.of<AuthProvider>(context, listen: false).isAuthenticated) { // Afficher un loader si on vérifie l'inscription et que l'user est loggé
      return const Padding(
        padding: EdgeInsets.all(16.0),
        child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [CircularProgressIndicator(strokeWidth: 2), SizedBox(width: 10), Text("Vérif. inscription...")])),
      );
    }
    // ... (votre logique ListView.separated)
